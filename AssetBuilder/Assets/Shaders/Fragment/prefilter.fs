...
uniform float currentRoughness;
...
vec3 ImportanceSampleGGX(vec2 Xi, vec3 N, float roughness){
	...
	//Convert from spherical to cartesian to get the halfway vector
...
void main(){
	...
	for(uint i = 0u; i < SAMPLE_COUNT; i++){
		...
		vec3 H = ImportanceSampleGGX(Xi, N, currentRoughness);
		vec3 L = normalize(2.0 * dot(V, H) * H - V);

		float NdotL = max(dot(N, L), 0.0);
		if(NdotL > 0.0){
			//Sample from environment's mip level based on roughness/pdf
			float D = DistributionGGX(N, H, currentRoughness);
			float NdotH = max(dot(N, H), 0.0);
            ...
            float mipLevel = currentRoughness == 0.0 ? 0.0 : 0.5 * log2(saSample / saTexel); 
